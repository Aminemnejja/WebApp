...
def gauss_jordan_dense(matrice ,b,n):
    ...
    for k in range(n):
        ...
        if aug_matrix[max_pivot_index, k] == 0:
            # Gestion du cas o˘ toutes les entrÈes sous la colonne k sont nulles
            continue
        else:
        ...
        # Diviser chaque ÈlÈment de la ligne par le pivot
...
def gauss_jordan_dense_symetrique(matrice, b,n):
    ...
    for k in range(n):
        # √âchelonner la colonne k
        pivot = aug_matrix[k, k]
        # Diviser chaque √©l√©ment de la ligne par le pivot
        for j in range(k+1,n+1):
            aug_matrix[k, j] /= pivot
        
        # √âliminer les autres colonnes en dessous et au-dessus de la diagonale
        # Il faut i != k !!! Diviser en deux boucles
        for i in range(k):
                ratio = aug_matrix[i, k]
                for j in range(k+1,n+1):
        ...
        for i in range(k+1,n):
            ratio = aug_matrix[i, k]
            for j in range(i,n):#travailler sur partie sup√©rieur car matrice sym√©trique
